...
# plt.title("Network with 30 nodes")
# plt.show()
...
def get_weight_two(i, j, N, kr, kc):
    if i != j and j in N:
        return (1 - get_weight_two(i, i, N, kr, kc)) / len(N)
    elif i == j:
        return c_w2 / get_v(i, kr, kc)
    else:
        return 0

# Go through iterations, calculate the updated measurement for each node at each cell, store the results
iterations = 50
...
# Get the mesh generated by the average of the node measurements
# plt.pcolormesh(field[:, :, 0], field[:, :, 1], x_avg, vmin=x_avg.min(), vmax=x_avg.max() )

# Plot the difference between the generated average values and the real values
diff = (x_avg - field[:, :, 2]).ravel()
